...
export default function Mapa() {
  ...
  const cargarPuntos = async () => {
    try {
      const respuesta = await fetch('http://127.0.0.1:8000/api/puntos');
  ...
  const actualizarEstado = async (puntoId: string, insumoId: string, nuevoEstado: string) => {
    try {
      await fetch(`http://127.0.0.1:8000/api/puntos/${puntoId}/insumos/${insumoId}`, {
  ...
  const guardarNuevoPunto = async (e: React.FormEvent) => {
    ...
    try {
      await fetch('http://127.0.0.1:8000/api/puntos', {
        method: 'POST',
  ...
  return (
    <div className="w-full h-full relative">
      {/* Mira fijamente el centro de la pantalla (Target visual para el usuario) */}
      ...
      <MapContainer 
        ...
        ref={mapRef} // Referencia para leer el centro del mapa
      >
          ...
          <Marker key={punto.id} position={[punto.latitud, punto.longitud]} icon={obtenerIconoPorTipo(punto.tipo)}>
            <Popup>
              <div className="font-sans min-w-[240px]">
                ...
                {/* Muestra la dirección y referencias si el usuario las ingresó */}
      ...
      {/* Botón Flotante: Ahora captura el centro de la pantalla inmediatamente */}
      ...
      {/* Modal del Formulario Manual/GPS */}
      {mostrarModal && (
